...
app.layout = dbc_container(
    html_div([
        ...
        dbc_row(
            [
                dbc_col(dcc_graph(id = "totalsumsy"), width=4),
                dbc_col(dcc_graph(id = "spreadsumsy"), width=4),
                dbc_col(html_div("One of three columns"), width=4),
            ]
        ),
        dbc_row(
            [
...
callback!(
    ...
    adata = [(balance,minimum),(balance,sum),(balance,maximum), (balance, mean),(balance, median)]
    ...
    print(data1[:,:])
    moneysum = Plot(data, x = :step, y = :sum_balance, group = :variable ,mode="markers")
    moneyspread = Plot(data1, x = :step, y = :value, group = :variable ,mode="markers")

    return (moneysum, moneyspread)
